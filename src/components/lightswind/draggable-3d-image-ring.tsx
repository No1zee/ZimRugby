...
export function ThreeDImageRing({ items, onSelectItem }: ThreeDImageRingProps) {
  ...
  return (
    <div className="w-full space-y-6 select-none">
      ...
      {/* Stage Section (Adaptive 3D Ring vs 2D Fallback) */}
      {isLowTierDevice || isSlowNetwork ? (
        <div className="relative w-full p-4 rounded-3xl bg-gradient-to-b from-[#010C07] via-[#00170E] to-[#010704] border border-[#006747]/30 shadow-2xl">
          <div className="flex items-center justify-between mb-4">
            <span className="px-3 py-1 bg-black/60 border border-[#006747]/40 text-white rounded-full text-[10px] font-black tracking-widest uppercase flex items-center gap-1.5 backdrop-blur-md">
              ...
              <span>MATCHDAY GALLERY (LIGHTWEIGHT)</span>
            </span>
          </div>
          ...
          <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-thin snap-x">
            {items.map((item, index) => (
              <div
                ...
                className="relative shrink-0 w-[200px] h-[260px] rounded-2xl overflow-hidden border border-white/15 hover:border-[#006747] snap-start cursor-pointer shadow-lg"
              >
                <Image
                  src={item.image}
                  alt={item.title}
