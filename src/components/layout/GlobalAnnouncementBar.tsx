    ...
    async function fetchAnnouncements() {
      try {
        ...
        if (!isDismissed) {
          ...
          // Delay visible trigger slightly so website loads first, then ribbon pops down cinematically
          const timer = setTimeout(() => {
            setIsVisible(true);
            // Trigger 1.5s attention-grabbing emerald glow pulse after landing
            const glowTimer = setTimeout(() => setShowGlow(true), 600);
            const glowOffTimer = setTimeout(() => setShowGlow(false), 2400);
            return () => {
        ...
        >
          ...
          <div className="max-w-[1440px] mx-auto px-6 py-2.5 flex items-center justify-between gap-4 relative z-10">
            ...
            <button 
              ...
              className="p-1 hover:bg-white/10 rounded-full transition-colors text-white/60 hover:text-white shrink-0"
