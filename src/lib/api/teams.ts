...
import { directusAsset, fetchFromDirectus, formatDayMonthYear } from "@/lib/directus/helpers";
...
export async function getTeamData(slug: string): Promise<Team | null> {
  ...
  return fetchFromDirectus<Team | null>(async () => {
    ...
    return {
      ...
      squad: (team.squad || []).map((s: any) => ({
        ...
        image: directusAsset(s.image) ?? (s.image_url || "/images/teams/player-placeholder.webp")
      })),
      matches: (team.matches || []).map((m: any) => ({
        opponent: m.opponent,
        opponentLogo: directusAsset(m.opponent_logo) ?? m.opponent_logo_url,
        date: m.date_label || formatDayMonthYear(m.date),
        venue: m.venue || "TBA",
        score: m.score,
        status: m.status || "upcoming"
      })),
      gallery: (team.gallery || []).map((img: any) =>
        directusAsset(img.image) ?? img.image_url
      )
    };
  }, () => teams[slug] || null, `team ${slug}`);
}
